...
import { LoggerColors } from 'src/berp'
import cs from 'cli-select'

export function createSelector(args: string[]): Promise<string> {
  return new Promise((res) => {
    cs({
      ...
      if (id) {
        try {
          res(JSON.parse(JSON.stringify(id)).value)
        } catch {
          res(undefined)
        }
      } else {
